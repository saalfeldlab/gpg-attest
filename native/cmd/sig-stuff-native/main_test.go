...
func TestHandle_sign(t *testing.T) {
	payload := base64.StdEncoding.EncodeToString([]byte("hello from sig-stuff test"))
	req := &protocol.Request{
		ID:      "12",
		Op:      "sign",
		KeyID:   "test@sig-stuff.dev",
		Payload: payload,
	}
	resp := handle(req)
	if !resp.OK {
...
func TestHandle_sign_verifiable(t *testing.T) {
	rawPayload := []byte("hello from sig-stuff test")
	payload := base64.StdEncoding.EncodeToString(rawPayload)
	req := &protocol.Request{
		ID: "12", Op: "sign", KeyID: "test@sig-stuff.dev", Payload: payload,
	}
	resp := handle(req)
	if !resp.OK {
		t.Fatalf("expected OK=true, got error: %s", resp.Error)
